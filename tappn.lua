getgenv().DNS = {
   ...
   Camlock = {
       Main = {
           ...
           Key = "C",
           UnlockKey = "V",
           SmoothLock = true, -- i recommend setting this to false.
           Smoothness = 0.5,
           PredictMovement = true, -- here, set to false if u want auto pred, set to true if u want to use ur own set
           Prediction = YOUR SET HERE,
           Shake = true, -- for shake.
           ShakeValue = 2.5,
           Parts = {"Head"} -- theres LowerTorso, Head, Uppertorso, etc.
       },
       FOV = {
           UseFOV = true,
           ShowFOV = false,
...
local function SendNotification(text)
   Notification:Notify(
       {Title = "TAPPN - By Yufi", Description = "TAPPN | "..text},
...
if uwuMain.Notifications then
   SendNotification("TAPPN - Setting Up..")
   wait(3.5)
   SendNotification("TAPPN - Finished Setting Up.")
end
...
uis.InputBegan:Connect(function(k,t)
   if not t then
       if k.KeyCode == Enum.KeyCode[uwuCamMain.Key:upper()] then
           ...
           if uwuMain.Notifications then
               SendNotification("Locked To Black Nigger: "..CTarget.Name)
           end
       ...
       elseif k.KeyCode == Enum.KeyCode[uwuSilentMain.Toggle:upper()] and uwuSilentMain == "Regular" then
           if SToggle then
               SToggle = false
               if uwuMain.Notifications then
                   SendNotification("Silent OFF")
               end
           else
               SToggle = true
               if uwuMain.Notifications then
                   SendNotification("Silent ON")
               end
           end
       end
   end
